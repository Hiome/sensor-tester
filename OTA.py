  ...
  #{
    ...
    if (sys.argv[i] == "-t" or sys.argv[i] == "-target") and len(sys.argv) >= i+2:
      if sys.argv[i+1].isdigit() and int(sys.argv[i+1])>0 and int(sys.argv[i+1])<=255:
        ...
        LOGln("TARGET invalid  (" + sys.argv[i+1] + "), must be 1-255.")
        exit(1)
    if (sys.argv[i] == "-l" or sys.argv[i] == "-lines") and len(sys.argv) >= i+2:
      if sys.argv[i+1].isdigit() and int(sys.argv[i+1])>0 and int(sys.argv[i+1])<=3:
        ...
        LOGln("LINESPERPACKET invalid  (" + sys.argv[i+1] + "), must be 1-3.")
        exit(1)
  #}
...
def waitForTargetSet(targetNode):
  ...
  now = millis()
  while millis()-now < 30000: # wait up to 30 seconds for programmer to read our command
...
try:
  ...
  ser = serial.Serial(SERIALPORT, BAUDRATE, timeout=1, xonxoff=True, rtscts=False, dsrdtr=False) #timeout=0 means nonblocking
...
EOF = ":00000001FF" #this should be the last line in any valid intel HEX file
    ...
    if (handshakeResponse == HANDSHAKE_OK):
      ...
      #{
        ...
        #{
          ...
          if LINESPERPACKET > 1 and currentLine[1:3] == '10' and currentLine[7:9] == '00':
          #{
            #check if next line != EOF, so we can bundle 2 lines
            nextLine = content[seq+1].strip()
            if nextLine != EOF and nextLine[1:3] == '10' and  nextLine[7:9] == '00':
            #{
              ...
              if LINESPERPACKET==3 and nextLine2 != EOF and nextLine2[1:3] == '10' and nextLine2[7:9] == '00':
              #{
                ...
                hexDataToSend = ':30' + hexDataToSend[3:len(hexDataToSend)-2] + nextLine[9:len(nextLine)-2] + nextLine2[9:len(nextLine)-2] + ('%0*X' % (2,checksum%256))
                bundledLines=3
              #}
              else:
              #{
                hexDataToSend = ':20' + hexDataToSend[3:len(hexDataToSend)-2] + nextLine[9:len(nextLine)-2] + ('%0*X' % (2,checksum%256))
                bundledLines=2;
              #}
            #}
          #}
      ...
      LOGln("FAIL: No response from Moteino programmer, is it connected to " + SERIALPORT)
      exit(1)
    else:
